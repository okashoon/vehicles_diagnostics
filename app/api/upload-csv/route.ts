...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      await client.query(
        `INSERT INTO vehicles_lookup
           (make_id, model_id, model_year_id,
            source_file, market, year_make, model_notes,
            module, vehicle_interface, obd_dlc_connect_cable,
            d2m_connect_cable, module_location)
