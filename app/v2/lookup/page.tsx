...
export default async function LookupV2({
  searchParams,
}: {
  ...
  return (
    <div className="min-h-screen bg-[#0f1117] p-8 text-[#e2e8f0]">
      <div className="mx-auto max-w-8xl">
        ...
        <div className="mb-8 flex items-start justify-between gap-4">
          <div>
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-[#3b82f6]/30 bg-[#3b82f6]/10 px-3 py-1">
              <span className="text-xs font-medium text-[#93c5fd]">Database</span>
            </div>
            <h1 className="text-2xl font-bold text-white">Vehicle Diagnostics Lookup</h1>
        ...
        {!isAuthenticated ? (
          ...
          <div className="rounded-2xl border border-dashed border-[#2d3748] bg-[#1e2330] py-16 text-center">
            <p className="text-sm text-[#475569]">
              {hasFilters ? "No records match these filters." : "No records found."}
            </p>
          </div>
        ) : (
          <div className="overflow-hidden rounded-2xl border border-[#2d3748] bg-[#1e2330]">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-[#2d3748] text-sm">
                  ...
                  <tr>
                    {COLUMNS.map((col) => (
                      <th
                        key={col.key}
                        scope="col"
                        className="whitespace-nowrap px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[#475569]"
                      >
                        {col.label}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-[#2d3748]">
                  {vehicles.map((row) => (
                    <tr key={row.id} className="transition-colors hover:bg-[#2d3748]/30">
                      {COLUMNS.map((col) => {
                        const value = row[col.key];
                        return (
                          <td
                            key={col.key}
                            className="max-w-xs truncate whitespace-nowrap px-4 py-3 text-[#94a3b8]"
                            title={value ?? ""}
                          >
                            {value ?? <span className="text-[#2d3748]">&mdash;</span>}
                          </td>
                        );
                      })}
                    </tr>
